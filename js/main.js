...
function coinEstate() {
  ...
  window.addEventListener('load', () => {
    this.startcoinEstate();
  });
}
coinEstate.prototype.ejs = function ejs(url, element, data) {
  if ($(`#${element}`).length) {
...
coinEstate.prototype.alertInfo = function alertInfo(message) {
  console.log(message);
...
coinEstate.prototype.alertDialog = function alertDialog(message) {
  console.log(message);
...
coinEstate.prototype.alertWarning = function alertWarning(message) {
  console.log(message);
...
coinEstate.prototype.alertError = function alertError(message) {
  console.log(message);
...
coinEstate.prototype.alertSuccess = function alertSuccess(message) {
  console.log(message);
...
coinEstate.prototype.txError = function txError(err) {
  ...
  utility.getBalance(this.web3, this.addrs[this.selectedAccount], (errBalance, resultBalance) => {
    ...
    if (this.connection.connection === 'RPC') {
      if (balance < 0.005) {
        this.alertError(
          `You tried to send an Ethereum transaction but there was an error. Your wallet's ETH balance (${balance} ETH) is not enough to cover the gas cost (Ethereum network fee). coinEstate sends 0.005 ETH with each transaction. This is an overestimate and the excess will get refunded to you. It's a good idea to send more than 0.005 so you can pay for not only this transaction, but also future transactions you do on coinEstate. The gas has to come directly from your Wallet (coinEstate has no physical way of paying gas from your deposited ETH).`);
        ga('send', {
          hitType: 'event',
          eventCategory: 'Error',
...
coinEstate.prototype.alertTxResult = function alertTxResult(err, txsIn) {
...
coinEstate.prototype.enableTooltipsAndPopovers = function enableTooltipsAndPopovers() {
  $('[data-toggle="popover"]').popover({ trigger: 'hover' });
  $('[data-toggle="tooltip"]').tooltip();
};
coinEstate.prototype.logout = function logout() {
...
coinEstate.prototype.createAccount = function createAccount() {
...
coinEstate.prototype.deleteAccount = function deleteAccount() {
...
coinEstate.prototype.selectAccount = function selectAccount(i) {
...
coinEstate.prototype.addAccount = function addAccount(newAddr, newPk) {
...
coinEstate.prototype.showPrivateKey = function showPrivateKey() {
...
coinEstate.prototype.addressLink = function addressLink(address) {
  return `https://${this.config.ethTestnet ? `${this.config.ethTestnet}.` : ''}etherscan.io/address/${address}`;
};
coinEstate.prototype.contractAddr = function contractAddr(addr) {
  this.config.contractcoinEstateAddr = addr;
  this.displayConnectionDescription(() => {});
  this.loading(() => {});
  this.refresh(() => {}, true, true);
};
coinEstate.prototype.displayAccounts = function displayAccounts(callback) {
...
coinEstate.prototype.displayLanguages = function displayLanguages(callback) {
...
coinEstate.prototype.selectLanguage = function selectLanguage(newLanguage) {
...
coinEstate.prototype.loadEvents = function loadEvents(callback) {
  let lastBlock = 0;
  Object.keys(this.eventsCache).forEach((id) => {
    const event = this.eventsCache[id];
    if (event.blockNumber > lastBlock && event.address === this.config.contractcoinEstateAddr) {
...
coinEstate.prototype.displayMyTransactions =
function displayMyTransactions(ordersIn, blockNumber, callback) {
  ...
  orders = orders.filter(order => order.order.contractAddr === this.config.contractcoinEstateAddr);
  ...
  events.forEach((event) => {
    try {
      if (
        event.event === 'Trade' &&
        event.address === this.config.contractcoinEstateAddr &&
...
coinEstate.prototype.displayVolumes = function displayVolumes(
  ...
  Object.keys(pairVolumes).forEach((pair) => {
    ...
    ordersFiltered = ordersFiltered.filter(
      order => order.order.contractAddr === this.config.contractcoinEstateAddr);
    // final order filtering and sorting
...
coinEstate.prototype.displayTradesAndChart = function displayTradesAndChart(callback) {
  // get the trade list
  const events = Object.values(this.eventsCache);
  const trades = [];
  events.forEach((event) => {
    if (event.event === 'Trade' && event.address === this.config.contractcoinEstateAddr) {
      if (event.args.amountGive.toNumber() > 0 && event.args.amountGet.toNumber() > 0) {
        // don't show trades involving 0 amounts
        let trade;
        if (
...
coinEstate.prototype.candlestickChart =
...
coinEstate.prototype.depthChart =
...
coinEstate.prototype.lineChart =
...
coinEstate.prototype.getOrdersByPair = function getOrdersByPair(tokenA, tokenB, callback) {
...
coinEstate.prototype.getReturnTicker = function getTopOrders(callback) {
...
coinEstate.prototype.getTopOrders = function getTopOrders(callback) {
...
coinEstate.prototype.displayOrderbook = function displayOrderbook(ordersIn, blockNumber, callback) {
  // only look at orders for the selected token and base
  let orders = ordersIn.filter(
    x =>
      (x.order.tokenGet.toLowerCase() === this.selectedToken.addr.toLowerCase() &&
...
coinEstate.prototype.displayTokensAndBases = function displayTokensAndBases(callback) {
...
coinEstate.prototype.displayAllBalances = function displayAllBalances(callback) {
    ...
    (token, callbackMap) => {
      if (token.addr === zeroAddr) {
        utility.call(
          ...
          this.config.contractcoinEstateAddr,
          'balanceOf',
...
coinEstate.prototype.transfer = function transfer(addr, inputAmount, toAddr) {
...
coinEstate.prototype.deposit = function deposit(addr, inputAmount) {
  let amount = new BigNumber(Number(utility.ethToWei(inputAmount, this.getDivisor(addr))));
  const token = this.getToken(addr);
  if (amount.lte(0)) {
      ...
      (err, result) => {
        ...
        if (amount.lte(result)) {
          utility.send(
            ...
            addr,
            'approve',
            [this.config.contractcoinEstateAddr, amount, { gas: this.config.gasApprove, value: 0 }],
            this.addrs[this.selectedAccount],
            this.pks[this.selectedAccount],
            this.nonce,
            (errSend, resultSend) => {
...
coinEstate.prototype.withdraw = function withdraw(addr, amountIn) {
...
coinEstate.prototype.order = function order(direction, amount, price, expires, refresh) {
...
coinEstate.prototype.publishOrder = function publishOrder(
      ...
      if (balance.lt(new BigNumber(amountGive))) {
        ...
        hash, this.pks[this.selectedAccount], (errSign, sig) => {
          if (errSign) {
            ...
            const order = {
              contractAddr: this.config.contractcoinEstateAddr,
              tokenGet,
              amountGet,
              tokenGive,
              amountGive,
...
coinEstate.prototype.cancelOrder = function cancelOrder(orderIn) {
...
coinEstate.prototype.trade = function trade(kind, order, inputAmount) {
  if (this.addrs[this.selectedAccount].slice(0, 39) === '0x0000000000000000000000000000000000000') {
    this.alertError(
      "You haven't selected an account. Make sure you have an account selected from the Accounts dropdown in the upper right.");
    ga('send', {
...
coinEstate.prototype.addPending = function addPending(err, txsIn) {
  const txs = Array.isArray(txsIn) ? txsIn : [txsIn];
  txs.forEach((tx) => {
...
coinEstate.prototype.updateUrl = function updateUrl() {
...
coinEstate.prototype.getDivisor = function getDivisor(tokenOrAddress) {
...
coinEstate.prototype.getToken = function getToken(addrOrToken, name, decimals) {
  let result;
...
coinEstate.prototype.loadToken = function loadToken(addr, callback) {
...
coinEstate.prototype.selectToken = function selectToken(addrOrToken, name, decimals) {
...
coinEstate.prototype.selectBase = function selectBase(addrOrToken, name, decimals) {
...
coinEstate.prototype.selectTokenAndBase = function selectTokenAndBase(tokenAddr, baseAddr) {
...
coinEstate.prototype.displayBuySell = function displayBuySell(callback) {
...
coinEstate.prototype.displayTokenGuidesDropdown = function displayTokenGuidesDropdown() {
  const tokens = this.config.tokens.map(x => x);
  tokens.sort((a, b) => (a.name > b.name ? 1 : -1));
...
coinEstate.prototype.displayHelpDropdown = function displayHelpDropdown() {
  this.ejs(`${this.config.homeURL}/templates/helpDropdown.ejs`, 'helpDropdown', {});
};
coinEstate.prototype.displayHelp = function displayHelp(name) {
  $('#helpBody').html('');
...
coinEstate.prototype.displayScreencast = function displayScreencast(name) {
  $('#screencastBody').html('');
...
coinEstate.prototype.displayConnectionDescription = function displayConnectionDescription() {
  this.ejs(`${this.config.homeURL}/templates/connectionDescription.ejs`, 'connection', {
    connection: this.connection,
    contracts: this.config.contractcoinEstateAddrs,
    contractAddr: this.config.contractcoinEstateAddr,
    contractLink: `https://${this.config.ethTestnet ? `${this.config.ethTestnet}.` : ''}etherscan.io/address/${this.config.contractcoinEstateAddr}`,
  });
};
coinEstate.prototype.displayTokenGuide = function displayTokenGuide(name) {
...
coinEstate.prototype.checkContractUpgrade = function checkContractUpgrade() {
  if (
    (!this.selectedContract || this.selectedContract !== this.config.contractcoinEstateAddr) &&
    (this.addrs.length > 1 ||
      (this.addrs.length === 1 && this.addrs[0].slice(0, 39) !== '0x0000000000000000000000000000000000000'))
  ) {
    this.alertDialog(
      '<p>coinEstate has a new smart contract. It is now selected.</p><p>Please use the "Smart Contract" menu to select the old one and withdraw from it.</p><p><a href="javascript:;" class="btn btn-default" onclick="alertify.closeAll(); bundle.coinEstate.displayHelp(\'smartContract\')">Smart contract changelog</a></p>');
  }
};
coinEstate.prototype.resetCaches = function resetCaches() {
...
coinEstate.prototype.loading = function loading(callback) {
  [
    'deposit',
    'withdraw',
    'buy',
...
coinEstate.prototype.refresh = function refresh(callback, forceEventRead, initMarket, token, base) {
  this.q.push((done) => {
    if (token && base) {
    ...
    this.selectedContract = this.config.contractcoinEstateAddr;
...
coinEstate.prototype.refreshLoop = function refreshLoop() {
  const self = this;
  function loop() {
    self.refresh(() => {
...
coinEstate.prototype.initDisplays = function initDisplays(callback) {
  this.loading(() => {});
...
coinEstate.prototype.loadWeb3 = function loadWeb3(callback) {
  this.config = config;
  // web3
...
coinEstate.prototype.initContracts = function initContracts(callback) {
  this.web3.version.getNetwork((error, version) => {
    ...
    // connection
    this.config.contractcoinEstateAddr = this.config.contractcoinEstateAddrs[0].addr;
    // get accounts
    ...
    utility.loadContract(
      ...
      this.config.contractcoinEstateAddr,
      (err, contractcoinEstate) => {
        this.contractcoinEstate = contractcoinEstate;
        utility.loadContract(
          this.web3,
...
coinEstate.prototype.startcoinEstate = function startcoinEstate() {
