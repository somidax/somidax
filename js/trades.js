...
function TradeUtil() {
  ...
  self.getContract = function getContract(callback) {
    self.get(`https://api.etherscan.io/api?module=contract&action=getabi&address=${addresscoinEstate}`, (err, data) => {
  ...
  self.getLog = function getLog(fromBlock, toBlock, callback) {
    function decodeEvent(item) {
      const eventAbis = self.contractcoinEstate.abi.filter(eventAbi => (
          eventAbi.type === 'event' &&
          item.topics[0] ===
            `0x${
              sha3(
      ...
      if (eventAbis.length > 0) {
        ...
        const event = new SolidityEvent(web3, eventAbi, addresscoinEstate);
    ...
    const url =
      `https://api.etherscan.io/api?module=logs&action=getLogs&address=${addresscoinEstate}&fromBlock=${fromBlock}&toBlock=${toBlock}`;
